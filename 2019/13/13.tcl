...
proc run {} {
    ...
    while {$return_code != 0} {
        ...
        switch $return_code {
            1 { #input
                set ball_x [lindex $ball_position 0]
                set paddle_x [lindex $paddle_position 0]
...
#                after 25
            ...
            2 { #output
                switch $output_state {
                    ...
                    "y" {
                        set output_state "tile"
                        set y $output
                    }
                    "tile" {
                        set output_state "x"
                        if {$output == 4} {
                            set ball_position [list $x $y]
                        }
                        if {$output == 3} {
                            set paddle_position [list $x $y]
                        }
                        if {$x == -1 && $y == 0} {
                            set score $output
                        } else {
...
run
set blocks 0
foreach {coord tile} [array get grid] {
    if {$tile == 2} {
        incr blocks
    }
}
