...
proc execute {program regs ip_reg} {
    set ip 0
    while 1 {
        # When on address 1, algoritm starts
        if {$ip == 1} {
            lset regs 0 [sum_dividers [lindex $regs 2]]
            return $regs
        }
...
puts "Register 0 after execution (reg0 = 0): [lindex $regs 0]"

# Initialize regs and run the program again
set regs [list 1 0 0 0 0 0]
set regs [execute $program $regs $ip_reg]
puts "Register 0 after execution (reg1 = 1): [lindex $regs 0]"
